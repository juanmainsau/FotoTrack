...
export function AlbumEditModal({ album, onClose, onSave }) {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      visibilidad: album.visibilidad || "publico",
      tags: album.tags || "",
    });
  ...
  async function loadImages(id) {
    try {
      ...
      const res = await fetch(`http://localhost:4000/api/imagenes/album/${id}?t=${Date.now()}`, {
        cache: "no-store",
      });

      const data = await res.json();
      setImagenes(data.ok ? data.imagenes || [] : []);
    } catch (err) {
  ...
  async function handleSyncIA() {
    if (!window.confirm("¿Re-escanear todo el álbum?")) return;
    ...
    try {
      ...
      if (data.ok) toast.success(data.message || "Sincronización terminada");
  ...
  async function handleUploadNewImages(e) {
    ...
    try {
      ...
      toast.success(`${files.length} imágenes añadidas.`);
      loadImages(album.id);
    } catch (err) {
            ...
            <div className="modal-body p-4 bg-white">
              <form id="editAlbumForm" onSubmit={handleSubmit}>
                <div className="row g-3">
                  ...
                  <div className="col-md-3">
                    <label className="form-label fw-bold small text-muted text-uppercase">Estado</label>
                    <select className="form-select border-primary-subtle" name="estado" value={form.estado} onChange={handleChange}>
                  ...
                  <div className="col-md-3">
                    <label className="form-label fw-bold small text-muted text-uppercase">Visibilidad</label>
                ...
                <div className="d-flex justify-content-between align-items-center mb-3 bg-light p-3 rounded border">
                  ...
                  <div className="d-flex gap-2">
                    <button type="button" className="btn btn-warning btn-sm fw-bold shadow-sm" onClick={handleSyncIA} disabled={syncing || uploading}>
                      {syncing ? "Procesando IA..." : "🤖 Re-escanear Caras"}
                    </button>

                    <input type="file" multiple accept="image/*" className="d-none" ref={fileInputRef} onChange={handleUploadNewImages} />

                    <button type="button" className="btn btn-primary btn-sm fw-bold shadow-sm" onClick={() => fileInputRef.current?.click()} disabled={uploading || syncing}>
                ...
                {loadingImages ? (
                  ...
                  <div className="row g-2 overflow-auto" style={{ maxHeight: "400px", padding: "10px", backgroundColor: "#f8f9fa", borderRadius: "8px" }}>
                    {imagenes.map((img) => (
                      <div key={img.idImagen} className="col-6 col-sm-4 col-md-3 col-lg-2">
                        <div className="card h-100 border-0 shadow-sm overflow-hidden position-relative">
                          <img src={img.rutaMiniatura} className="card-img-top" style={{ aspectRatio: "1/1", objectFit: "cover" }} alt="" />
                          <button type="button" className="btn btn-danger btn-sm position-absolute top-0 end-0 m-1 p-1 lh-1" style={{ borderRadius: "50%", width: "24px", height: "24px" }} onClick={() => handleDeleteImage(img)}>
                            ✖
                          </button>
                        </div>
                      </div>
                    ...
                    {imagenes.length === 0 && (
                      <p className="text-center text-muted py-4 w-100">
